...
_Long answer:_ Heybarn works as a template for a marketplace user interface. It can be used as a
marketplace client out of the box but is intended to be modified to match the requirements of each
marketplace built on top of the Flex API. Also, marketplaces that do not use Heybarn as a template
...
Marketplace features included in Heybarn:
...
- **Transaction process**: Heybarn uses nightly booking.
...
- `src/marketplace.css`, `src/marketplaceIndex.css`, `src/marketplaceIndex.css` define marketplace
  ...
  Property Sets.
- `src/translations/en.json` all English translations used in Heybarn template app. (A good place
